    ...
    <div class="mt-8">
        <x-settings heading="Manage Meal types">
            ...
            <div class="overflow-x-auto mt-4">
                        ...
                        @foreach($mealTypes as $mealType)
                            <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700">
                                ...
                                <td class="w-1/6 px-6 py-4">
                                    <p class="text-sm">{{$mealType->created_at}}</p>
                                </td>
                                <td class="px-6 py-4">
                                    <a href="/admin/meal-types/{{$mealType->id}}/edit"
                                       class="text-blue-500 hover:text-blue-600">Edit</a>
                                </td>

                                <td class="px-6 py-4">
                                    <form action="/admin/meal-types/{{$mealType->id}}" method="post">
                                        @csrf
                                        @method('DELETE')

                                        <button class="text-red-500 hover:text-red-600"
                                                onclick="return confirm('By clicking \'ok\' you confirm that you are aware that the meal type \'{{$mealType->name}}\' will be permanently deleted?')">
                                            Delete
                                        </button>
                ...
                @else
                    <p class="text-center text-lg text-gray-500">No meal types yet. Come back later</p>
                @endif
            </div>
            <div class="p-3">
                {{$mealTypes->links()}}
