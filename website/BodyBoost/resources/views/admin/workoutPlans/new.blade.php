...
@section('content')
    ...
    <div class="flex justify-center mt-8 p-8">
        <div class="p-8 w-3/4 justify-center bg-gray-50 rounded-xl">
            <h1 class="text-xl text-red-500">Create Workout plan</h1>
            <form method="POST" action="/admin/workout-plans/create" enctype="multipart/form-data">
                @csrf
                @method('POST')
                ...
                <div class="mt-3">
                    ...
                    <x-form.formInput name="workoutId" type="number" placeholder="1, 2, 3..."/>
                    @error('workoutId')
                    <p class="text-red-500 text-xs mt-2">{{$message}}</p>
                    @enderror
                </div>
