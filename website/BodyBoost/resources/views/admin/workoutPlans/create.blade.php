...
@section('content')
    <div class="mt-8">
        <x-settings heading="Manage Workout Plans">
            ...
            <div class="overflow-x-auto mt-4">
                        ...
                        @foreach($workoutPlans as $workoutPlan)
                            <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700">
                                ...
                                <td class="w-1/6 px-6 py-4">
                                    <p class="text-sm">{{$workoutPlan->created_at}}</p>
                                </td>
                                <td class="px-6 py-4">
                                    <a href="/admin/workout-plans/{{$workoutPlan->id}}/edit"
                                       class="text-blue-500 hover:text-blue-600">Edit</a>
                                </td>

                                <td class="px-6 py-4">
                                    <form action="/admin/workout-plans/{{$workoutPlan->id}}" method="post">
                                        @csrf
                                        @method('DELETE')
                ...
                @else
                    <p class="text-center text-lg text-gray-500">No Workout plans yet. Come back later</p>
                @endif
            </div>
            <div class="p-3">
                {{$workoutPlans->links()}}
