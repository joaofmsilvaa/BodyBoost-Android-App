...
@section('content')
    ...
    <div class="flex justify-center mt-8 p-8">
        <div class="p-8 w-3/4 justify-center bg-gray-50 rounded-xl">
            <h1 class="text-xl text-red-500">Edit Dietary type</h1>
            <form method="POST" action="/admin/dietary-types/{{$dietaryType->id}}" enctype="multipart/form-data">
                @csrf
                @method('PATCH')

                <label for="name" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Name</label>
                <x-form.formInput name="name" value="{{$dietaryType->name}}"/>
                @error('name')
                <p class="text-red-500 text-xs mt-2">{{$message}}</p>
                @enderror

                <label for="slug" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Slug</label>
                <x-form.formInput name="slug" value="{{$dietaryType->slug}}"/>
                @error('slug')
                <p class="text-red-500 text-xs mt-2">{{$message}}</p>
                @enderror
